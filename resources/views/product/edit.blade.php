        ...
        <form action="{{ route('product.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="w-full flex flex-col items-center">
            ...
            <div class="w-full max-w-4xl mb-8">
                <div class="flex items-center gap-2 mb-6 text-sm">
                    ...
                    <a class="text-slate-500 dark:text-[#9d9fb9] hover:text-primary transition-colors font-medium" href="{{ route('produc.index') }}">Products</a>
                    <span class="material-symbols-outlined text-[16px] text-slate-400 dark:text-[#56586d]">chevron_right</span>
                    <span class="text-slate-900 dark:text-white font-medium">Edit Product</span>
                </div>
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h1 class="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white tracking-tight mb-2">Edit: {{ $product->name }}</h1>
                        <p class="text-slate-500 dark:text-[#9d9fb9] text-base">Update the information for this listing.</p>
                    </div>
                    <div class="flex gap-3">
                        <a href="{{ route('produc.index') }}" class="px-5 py-2.5 rounded-lg border border-slate-200 dark:border-[#3b3d54] text-slate-700 dark:text-[#9d9fb9] font-medium hover:bg-slate-50 dark:hover:bg-[#282a39] transition-colors text-sm flex items-center justify-center">Cancel</a>
            ...
            <div class="w-full max-w-4xl bg-white dark:bg-[#1c1c27] rounded-xl border border-slate-200 dark:border-[#282a39] shadow-sm overflow-hidden">
                <div class="p-8">
                    ...
                    <div class="mb-10">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <textarea name="description" id="description" class="w-full rounded-lg bg-slate-50 dark:bg-[#111218] border border-slate-200 dark:border-[#3b3d54] px-4 py-3 text-base text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none resize-none" placeholder="Características del producto..." rows="4" >{{ old('description', $product->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-10">
                        ...
                        <div class="w-full">
                            <label class="text-slate-700 dark:text-slate-300 text-sm font-medium uppercase tracking-wider mb-2 block">Upload Image (Optional)</label>
                            
                            {{-- Mostrar imagen actual si existe --}}
                            @if($product->image && $product->image !== 'no hay ruta')
                                <div class="mb-4">
                                    <p class="text-sm text-slate-500 mb-2">Current Image:</p>
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="h-32 object-contain rounded-lg border border-slate-200 dark:border-[#3b3d54] bg-slate-50 dark:bg-[#111218] p-2">
                                </div>
                            @endif
